...
CSV.write(datadir("sim_data", "control_individ_frame.csv"), final_individ_frame)
CSV.write(datadir("sim_data", "control_state_frame.csv"), final_state_frame)
...
# save the trees 
CSV.write(datadir("sim_data", "control_iso50_trees.csv"), final_iso50_tree)
...
# save the trees 
CSV.write(datadir("sim_data", "control_iso100_trees.csv"), final_iso100_tree)
...
# save the trees
CSV.write(datadir("sim_data", "control_het50_trees.csv"), final_het50_tree)
...
# save the trees
CSV.write(datadir("sim_data", "control_het100_trees.csv"), final_het100_tree)

### update sim_dict.csv ###
sim_dict = CSV.read(datadir("sim_data", "sim_dict.csv"), DataFrame)
sim_dict = sim_dict[.!(sim_dict.sim_id .∈ Ref([9, 10, 11, 12])), :]
new_sim_dict = DataFrame(
    sim_id = [9,10,11,12],
    sim = ["control_iso50", "control_iso100", "control_het50", "control_het100"],
    individ_frame_file = ["control_individ_frame.csv", "control_individ_frame.csv", "control_individ_frame.csv", "control_individ_frame.csv"],
    state_frame_file = ["control_state_frame.csv", "control_state_frame.csv", "control_state_frame.csv", "control_state_frame.csv"],
    tree_file = ["control_iso50_trees.csv", "control_iso100_trees.csv", "control_het50_trees.csv", "control_het100_trees.csv"],
    sim_julia_file = ["sim_control_scenario.jl", "sim_control_scenario.jl", "sim_control_scenario.jl", "sim_control_scenario.jl"],
...
write(datadir("sim_data", "control_iso50_simnum1.tree"), newick_tree)
# sim 10
trees = CSV.read(datadir("sim_data", "control_iso100_trees.csv"), DataFrame)
trees.og_ids = [eval(Meta.parse(trees.og_ids[i])) for i in 1:size(trees, 1)]
# filter for sim == 1
coal_and_samp_tree = trees[trees.sim .== 1, :]
...
newick_tree = construct_newick_tree(coal_and_samp_tree, reverse_samp_lin)
# save the newick tree as .tree
write(datadir("sim_data", "control_iso100_simnum1.tree"), newick_tree)
# sim 10. 3
trees = CSV.read(datadir("sim_data", "control_iso100_trees.csv"), DataFrame)
trees.og_ids = [eval(Meta.parse(trees.og_ids[i])) for i in 1:size(trees, 1)]
# filter for sim == 1
...
newick_tree = construct_newick_tree(coal_and_samp_tree, reverse_samp_lin)
# save the newick tree as .tree
write(datadir("sim_data", "control_iso100_simnum3.tree"), newick_tree)

# sim 11
trees = CSV.read(datadir("sim_data", "control_het50_trees.csv"), DataFrame)
trees.og_ids = [eval(Meta.parse(trees.og_ids[i])) for i in 1:size(trees, 1)]
# filter for sim == 1
coal_and_samp_tree = trees[trees.sim .== 1, :]
...
newick_tree = construct_newick_tree(coal_and_samp_tree, reverse_samp_lin)
# save the newick tree as .tree
write(datadir("sim_data", "control_het50_simnum1.tree"), newick_tree)
# sim 12
trees = CSV.read(datadir("sim_data", "control_het100_trees.csv"), DataFrame)
trees.og_ids = [eval(Meta.parse(trees.og_ids[i])) for i in 1:size(trees, 1)]
# filter for sim == 1
coal_and_samp_tree = trees[trees.sim .== 1, :]
...
newick_tree = construct_newick_tree(coal_and_samp_tree, reverse_samp_lin)
# save the newick tree as .tree
write(datadir("sim_data", "control_het100_simnum1.tree"), newick_tree)
#sim 12 num 3
trees = CSV.read(datadir("sim_data", "control_het100_trees.csv"), DataFrame)
trees.og_ids = [eval(Meta.parse(trees.og_ids[i])) for i in 1:size(trees, 1)]
# filter for sim == 1
...
newick_tree = construct_newick_tree(coal_and_samp_tree, reverse_samp_lin)
# save the newick tree as .tree
write(datadir("sim_data", "control_het100_simnum3.tree"), newick_tree)