...
# using the no-rejection stochastic trajectories (no I[end] >= 5/6 filter)
...
stoch_frame <- read_csv(here::here("data", "compare_data", "compare_ei_state_frame_norejection.csv"))

set.seed(14578)
randnums <- sample.int(100, 10)
...
stoch_frame_r15 <- read_csv(here::here("data", "compare_data", "compare_ei_state_frame_norejection_r=1.5.csv"))

set.seed(14578)
randnums_r15 <- sample.int(100, 10)
...
stoch_frame_r13 <- read_csv(here::here("data", "compare_data", "compare_ei_state_frame_norejection_r=1.3.csv"))

set.seed(14578)
randnums_r13 <- sample.int(100, 10)
...
patchwork_plot <- (count_plot_orig + count_plot_r15 + count_plot_r13) +
  plot_annotation(title = "ODE vs Stochastic (No Rejection) EI Trajectories Varying R0")
ggsave(here::here("figures", "odeEI_vs_stochEI_norejection_allR0.pdf"),
