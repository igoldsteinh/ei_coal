...
### time transformation, rejecting bad pop, use ODE result instead of true traj ###
...
CSV.write(datadir("compare_data", "compare_ei_tt_odebadpop_results.csv"), results_df5_ode)
...
init_nE = 0
max_time = last_samp_time
results5_ode_vargrid = Array{Float64}(undef, 1000, 5)  # 10000 rows, 5 columns
# need a new comp_frame
# calculates the solutions to the EI ode system
include(srcdir("calc_ei_trajectoriesv2.jl"))
# needed to calcluate the solutions to the EI ode system
include(srcdir("core_ei_func.jl"))
...
for s in 1:1000
    ...
    comp_frame.reverse_time = abs.(comp_frame.time .- last_samp_time)
    reverse_times = sort(comp_frame.reverse_time)
    comp_times = comp_frame.time[2:end]
    E_traj = zeros(length(comp_times))
    I_traj = zeros(length(comp_times))
    init_e0 = 0
    init_i0 = 1
    ...
    new_row = [s; coal_times...]
    # Add the new row to the results array
    results5_ode_vargrid[s, :] = new_row
end
column_names = [:iteration, :coal_time1, :coal_time2, :coal_time3, :coal_time4]

results_df5_ode_vargrid = DataFrame(results5_ode_vargrid, column_names)
results_df5_ode_vargrid.diff1 = results_df5_ode_vargrid.coal_time1
results_df5_ode_vargrid.diff2 = results_df5_ode_vargrid.coal_time2 .- results_df5_ode_vargrid.coal_time1
results_df5_ode_vargrid.diff3 = results_df5_ode_vargrid.coal_time3 .- results_df5_ode_vargrid.coal_time2
results_df5_ode_vargrid.diff4 = results_df5_ode_vargrid.coal_time4 .- results_df5_ode_vargrid.coal_time3
CSV.write(datadir("compare_data", "compare_ei_tt_odebadpop_vargrid_results.csv"), results_df5_ode_vargrid)
