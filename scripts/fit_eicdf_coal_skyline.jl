# REDO WITH SKYLINE AND FIVE ALPHA VALUES
using CSV
using DataFrames
using DrWatson
using Distributions
...
include(srcdir("node_simple", "sample_nodescdf_andparams_skyline.jl"))
include(srcdir("node_simple", "sample_ess_simple_skyline.jl"))
...
include(projectdir("src/load_dp_ei_coal_skyline.jl"))
# choose alpha_times 
change_grid = 7.0
samp_time = maximum(tree.time) 
# these are the change times from the last sampling time in BEAST2 (fit_bdmm_control.xml)
backtimes = vcat(30.6, 61.2, 91.8, 122.4)
change_times = sort!(samp_time .- backtimes)
alpha_times = vcat(0,change_times)
...
string("ei_cdf_skyline_sim", sim_id, "_simnum", sim_num, ".csv")), my_samples_frame)
