...
function calc_coal_pdf_simplev2(num_lineages::Int, coal_time::Float64, init_dist::AbstractVector{Float64}, start_time::Float64, 
    ...
    A_matrix2::AbstractMatrix{Float64}, L_matrix::AbstractMatrix{Float64}, new_inverse_term::AbstractMatrix{Float64}, cdf::AbstractVector{Float64}, 
    pdf::AbstractVector{Float64}, my_method, my_cache, temp_vec::AbstractVector{Float64}) 
...
function sample_internal_nodes_safev2(num_lineages, est_times, coal_times, init_dist, start_time, last_samp_time, 
    ...
    @views for t in 1:(length(est_times)-1)
        if active_est_time == next_coal_time 
            ...
            gamma, alpha_t, E, I, id_matrix[1:(active_lineages + 1), 1:(active_lineages + 1)], 
            A_matrix[1:(active_lineages + 1), 1:(active_lineages + 1)], 
            A_matrix2[1:(active_lineages + 1), 1:(active_lineages + 1)],
            L_matrix[1:(active_lineages + 1), 1:(active_lineages - 1)], 
            new_inverse_term[1:(active_lineages + 1), 1:(active_lineages - 1)], 
            cdf[1:(active_lineages-1)], pdf[1:(active_lineages-1)],
            ...
            if log(active_pdf[state]) == -Inf
                println("active_pdf",active_pdf)
                println("active_cdf", active_cdf)
                println("t", t)
                println("state", state)
                println("sampled_node_states", sampled_node_states)
                println("active_lineages", active_lineages)
                println("active_est_time", active_est_time)
                println("active_start_time", active_start_time)
                println("gamma", gamma)
                println("alpha_t", alpha_t)
                println("E", E)
                println("I", I)
                println("init_dist", init_dist)
                error("active_pdf[state] is -Inf")
            end
...
function sample_internal_nodesv2!(num_lineages::Int, est_times::AbstractVector{Float64}, coal_times::AbstractVector{Float64}, 
    ...
    expv_cache_dict::Dict{Int, Any}, A_matrix::AbstractMatrix{Float64}, A_matrix2::AbstractMatrix{Float64}, L_matrix::AbstractMatrix{Float64}, 
    id_matrix::AbstractMatrix{Float64}, new_inverse_term::AbstractMatrix{Float64}, cdf::AbstractVector{Float64}, pdf::AbstractVector{Float64}, my_method, 
    temp_vec::AbstractVector{Float64}, L_vec::AbstractVector{Float64}, my_vec::AbstractVector{Float64}, sampled_node_states::Vector{Int}, 
    ...
    @views for t in 1:(length(est_times)-1)
        if active_est_time == next_coal_time 
            ...
            if active_lineages + 1 > mat_size && delta_t < tstep_cutoff
                ...
                state, active_start_time, gamma, alpha_t, E, I)
                active_start_state = sampled_node_states[t]
                if lik_vec[active_start_state] < 0
                    println("active_pdf",active_pdf)
                    println("active_cdf", active_cdf)
                    println("lik_vec", lik_vec)
                    println("t", t)
                    println("state", state)
                    println("active_start_state", active_start_state)
                    println("sampled_node_states", sampled_node_states)
                    println("active_lineages", active_lineages)
                    println("active_est_time", active_est_time)
                    println("active_start_time", active_start_time)
    ...
    ### safety check, compare against the other way of calculating the likelihood
...
#     if ll_check != log_lik
#         println("ll_check= ", ll_check)
#         println("log_lik= ", log_lik)
#         println("ll_vec_check= ", ll_vec_check)
#         println("ll_vec= ", ll_vec)
#         println("sampled_node_states= ", sampled_node_states)
#         println("reverse_E= ", reverse_E)
#         println("reverse_I= ", reverse_I)
#         println("alpha_vec= ", alpha_vec)
#         println("gamma= ", gamma)
#         error("log likelihoods do not match")
#     end
